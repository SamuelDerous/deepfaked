...
public class ControlsPane extends VBox {
    ...
    public ControlsPane(EmbeddedMediaPlayer mediaPlayer) {
        ...
        vol.valueProperty().addListener((obs, oldValue, newValue) -> {
            if(vol.isPressed()) {
                mediaPlayer.audio().setVolume((int) vol.getValue());
            }
        });
    }
    ...
    private Button createButton(String name, String icon) {
        ...
        String url = String.format("/assets/icons/buttons/%s.png", icon);
