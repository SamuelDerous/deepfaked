...
public class IntroView extends PrntView {
    ...
    public void createButtons() {
        ...
        btnYes.setOnAction((event) -> {
            IntroController.getIntroStage().setScene(InfoFactory.createMissionView().playScene("Geweldig! Dit zijn de missies die we voor je hebben verzameld:"));
        });
    ...
    private EventHandler<KeyEvent> createYesNoEvent() {
        EventHandler<KeyEvent> yesNoHandler = (KeyEvent key) -> {
            if (key.getCode() == KeyCode.J) {
                LoginController.getIntroStage().setScene(InfoFactory.createMissionView().playScene("Geweldig! Dit zijn de missies die we voor je hebben verzameld:"));
